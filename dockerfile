...
# استخدم CMD بدلاً من ENTRYPOINT لتجنب مشاكل الصياغة
CMD /bin/bash -c '\
  ...
  if test -n "$DATABASE_URL"; then \
    ...
    echo "{\"Database\":{\"Type\":\"PostgreSQL\",\"ConnectionString\":\"$CONN_STRING\",\"Provider\":\"Npgsql\",\"AutoMigrate\":true}}" > /app/appsettings.json; \
